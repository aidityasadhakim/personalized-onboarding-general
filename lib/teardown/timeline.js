...
export function buildTimeline(report, requestedHost) {
  ...
  push(700, { type: "log", text: "Drafted EC-01 with 3 mockups and a run-length estimate" });
  push(300, { type: "stage", stage: "ideas", status: "done" });
  push(150, {
    type: "message",
    message: {
      role: "agent",
      text: "Drafted EC-01: the gate joins the conversation. Keep the first output whole, then gate only the next artifact, counted and blurred.",
  ...
  push(150, {
    ...
    message: {
      role: "agent",
      text: "Your personalized first run for Enrich Labs is ready. Pick an intent, get one real win, and meet the ask only after the win is on screen. Try it in the sidebar.",
