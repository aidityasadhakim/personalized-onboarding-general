...
const routes = [
  ...
  },
  {
    ...
    reply: (r) => ({
      text: `Buyers compare Enrich Labs with ${r.profile.competitors.slice(0, -1).join(", ")}, and ${r.profile.competitors.at(-1)} already ranks for “Enrich Labs alternatives”. Against stronger activation paths, Enrich Labs loses on timing, not effort: the proof arrives late, the gate interrupts the first read, and the CTA asks for a hire. The funnel index and pricing are under Context › Competitors.`,
      tab: "competitors",
    }),
  },
  {
  ...
  },
  {
    ...
    reply: () => ({
      text: "EC-01 moves the gate into the conversation. At 500 visitors a day and 20% conversion, a big win reads in about 2 days and a 10% lift in about a month. Adjust the calculator under Test › Setup to match their traffic.",
      attachments: [{ type: "idea", id: "EC-01" }],
      tab: "setup",
    }),
  },
...
const fallbackTopic = {
  ...
  setup: "EC-01, or how long the test runs",
...
export const suggestionsFor = {
  ...
  ideas: ["Which issue do we ship first?", "How long would EC-01 run?", "Why the Tinder reference?"],
  setup: ["How long would EC-01 run?", "Why the Tinder reference?", "Show the onboarding prototype"],
