...
import { describeMove, routeOf } from "./rules";
...
export function renderMove(move, state, plan) {
  switch (move.kind) {
    ...
    case "plan":
      // On the fast lane this screen is also the offer, so it carries the pricing.
      return { kind: "plan", id: "plan", plan, withPricing: routeOf(state.profile) === "ready" };
    case "paywall":
